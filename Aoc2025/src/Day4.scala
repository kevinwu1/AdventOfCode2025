...
"""
  ...
  def part1(input: String): Int = {
    ...
    grid.positions
      .count(p => {
        grid(p) == "@" &&
        adjacent(p).map(grid(_)).count(_ == "@") < 4
      })
  }
  ...
  def part2(input: String): Int = {
    ...
    // @tailrec
    @logged
    def removeUntilCant(total: Int = 0): Int = {
      val toRemove = grid.positions
        .filter(p => {
          grid(p) == "@" &&
          adjacent(p).map(grid(_)).count(_ == "@") < 4
        })
      ...
      if (toRemove.size == 0) {
        ...
        toRemove.foreach(t => grid(t) = ".")
        removeUntilCant(total + toRemove.size)
      }
    }
    removeUntilCant()
  }
